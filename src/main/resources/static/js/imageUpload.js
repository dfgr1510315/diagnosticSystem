(function(window, document) {
	...
	Cupload.prototype = {
		...
		initDom: function() {
			...
			if (this.opt.data) {
				this.copyDate(this.opt.data,this.oldImage);
		...
		copyDate:function(from,to){
			for (let i=0;i<from.length;i++){
				let src = from[i].split('/');
				to[i] = src[src.length-1];
			}
		},
		...
		uploadImage: function() {
			this.inputFile[this.i] = this.uploadInput.files[0];
		...
		//{src, name, width, height, state}
		//src, name, width, height, state = true
		...
		deleteImage: function(n) {
			...
			this.inputFile.splice(n,1);
			if (n>=this.oldImage.length) n = this.oldImage.length-1;
			this.oldImage.splice(n,1);
			this.removeUploadBox();
			if (this.imageList.children.length < this.opt.num) {
				this.createUploadBox()
			}
